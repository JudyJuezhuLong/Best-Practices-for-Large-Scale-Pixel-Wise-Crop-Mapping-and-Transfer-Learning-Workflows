...
source_scene_id1 = '22_34'
source_scene_id2 = '28_29'
source_scene_id = source_scene_id1 + '_' + source_scene_id2

target_scene_id = 'Serbia'
scene_id = source_scene_id+'_'+target_scene_id
best_models_directory1 = '/mnt/mridata/Anonymouslong/best_practice_pixel/22_34_28_29_10_folds'
...
def make_data_loader(x,y,shuffle,batch_size,num_workers,_collate_fn,drop_last, sampler=None):
    if sampler:
        return DataLoader(
...
def _eval_perf(net, dataloader, best_loss, device):
    ...
    if val_loss < best_loss:
        ...
        # save_checkpoint(net, optimizer, epoch, val_loss, os.path.join('./output_Anonymous_fine_tune_'+scene_id+'site', yy+"_"+preprocessing_method+"_"+model_type+"_"+sample_size+"_best.pth"))
        # save_checkpoint(classifier, optimizer, epoch, val_loss, 'model_checkpoint.pth')
    return val_loss, val_acc, best_loss

def _eval_perf2(net, dataloader, best_loss, device):
    # Validation phase
    net.eval()
    val_loss = 0
    val_correct = 0
    ...
    val_loss /= len(val_loader)
    val_acc = val_correct / total_samples
    print(f"Epoch {epoch + 21}, Validation Loss: {val_loss:.4f}")
    print(f"Epoch {epoch + 21}, Validation Acc: {val_acc:.4f}")
...
def _eval_perf_f1(net, dataloader, best_loss, device):
    best_f1, patience, wait = -1, 2, 0
    val_loss = 0
    val_correct = 0
    total_samples = 0
    # Validation phase
    ...
    with torch.no_grad():
        for j, s in enumerate(dataloader):
            ...
            val_loss += criterion(y_s, labels_s).item()
            val_correct += (yt_pred == labels_s).sum().item()
            all_labels.extend(labels_s.cpu().numpy())
            all_preds.extend(yt_pred.cpu().numpy())
            total_samples += labels_s.size(0)
    ...
    if f1 > best_f1:
        best_f1, wait = f1, 0
        # torch.save(net.state_dict(), "stageB_best.pt")
        best_loss = val_loss
        save_checkpoint(net, optimizer, epoch, val_loss, os.path.join(f'./output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth"))
    else:
        wait += 1
...
train_size_target = 4000 # Be used to select the Train set and Valid set (4000 in total)
...
for fold_i in range(n_folds): 
    ...
    # balanced_loader = make_balanced_data_loader(x_train,y_train, shuffle=True,batch_size=batch_size,num_workers=num_workers,drop_last=True,_collate_fn=_collate_fn)

    # Step A
    def make_balanced_data_loader(x, y, shuffle, batch_size, num_workers, drop_last, _collate_fn, sampler=None):
        train_dataset = CropMappingDataset(x, y)
        targets = torch.tensor(y)

        # indices for each class
        class_indices = [torch.where(targets == class_label)[0] for class_label in range(len(np.unique(y)))]

        # choose min_class = minimum samples per class
        min_class_count = min(len(indices) for indices in class_indices)
        subset_indices = torch.cat([
            indices[torch.randperm(len(indices))[:min_class_count]]
            for indices in class_indices
        ])

        balanced_ds = Subset(train_dataset, subset_indices)
    ...
    for name, param in classifier.named_parameters():
        param.requires_grad = name.startswith("fc2")
    ...
    for epoch in range(20):                    
        ...
        for idx, batch in enumerate(balanced_loader):
            optimizer.zero_grad()
            x = batch['x'].to(device)
            y = batch['y'].to(device)
    ...
    # R2
    class_counts = np.unique(y_train,return_counts=True)[1] # corn, soy, other
    # inv_freq = 1.0 / class_counts # [1/2, 1/1, 1/7]
    # inv_freq = inv_freq / inv_freq.sum() * len(class_counts) # optional rescale
    # inv_freq = torch.tensor(inv_freq, dtype=torch.float32, device=device)
    # criterion = nn.CrossEntropyLoss(weight=inv_freq.to(device))

    # R3
    class_counts = torch.tensor(class_counts, dtype=torch.float32, device=device)
    w      = torch.sqrt(1.0 / class_counts)              # [0.022, 0.032, 0.017]
    w      = w / w.mean()                          # rescale so mean ≈ 1
    
    criterion = nn.CrossEntropyLoss(weight=w.to(device))

    sample_wts = (1.0 / class_counts)[y_train]        # weight per example
    sampler    = torch.utils.data.WeightedRandomSampler(
                    weights = sample_wts,
                    num_samples = len(y_train),      # keep epoch length unchanged
                    replacement = True)

    train_loader = make_data_loader(x_train, y_train, shuffle=False,batch_size=batch_size,num_workers=num_workers,_collate_fn=_collate_fn,drop_last=True, sampler=sampler)# =train_dataloader
    ...
    for epoch in range(20):
        ...
        val_loss, val_correct, best_loss = _eval_perf2(classifier, val_loader, best_loss, device)
        # val_loss, val_correct, best_loss = _eval_perf_f1(classifier, val_loader, best_loss, device)
        # cls_accs_ls.append(round(cls_accs.val,2)*0.01)
        # loss_ls.append(round(losses.val,2))
        cls_accs_ls.append(val_correct)
        loss_ls.append(val_loss)
    ...
    fine_tun_checkpoint_path=os.path.join(f'./output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth")
    classifier,optimizer = load_checkpoint(fine_tun_checkpoint_path)
    with torch.no_grad():
