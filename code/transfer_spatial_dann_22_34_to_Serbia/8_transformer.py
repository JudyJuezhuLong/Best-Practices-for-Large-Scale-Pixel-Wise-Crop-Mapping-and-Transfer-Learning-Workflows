...
def main(args: argparse.Namespace):
    ...
    x_source, _, y_source, _ = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=250000, random_state=17)
    ...
    # check_x = x_source#[batch_size*3:batch_size*4,:,:]
    # check_y = y_source#[batch_size*3:batch_size*4]

    # print("check x nan",np.unique(np.isnan(check_x)))
    # print(" x min",np.min(check_x), "x max",np.max(check_x))
    # print("y values",np.unique(check_y))
    # ------------------------------- 
    
    scaler, x_source, x_target =normalize_without_scaler(x_source, x_target)
    # x_source, x_target = min_max_normalize(x_source, x_target)

    # print(" x_source_std min",np.min(x_source), "x_source_std max",np.max(x_source))
    # print(" x_target_std min",np.min(x_target), "x_target_std max",np.max(x_target))
    ...
    classifier = build_model(input_dim = input_feature_size, num_classes = num_classes, seq_len = x_source.shape[1]).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for j, s in enumerate(train_source_loader):
            ...
            # edited by Anonymous ================ end =============
            # print(torch.min(f_s.cpu()), torch.max(f_s.cpu()))
            # print(cls_loss.cpu())

            # print(torch.min(f_t.cpu()), torch.max(f_t.cpu()))
