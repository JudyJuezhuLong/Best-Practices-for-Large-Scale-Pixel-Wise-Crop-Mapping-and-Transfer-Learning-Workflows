...
# best_models_directory1 = "/mnt/mridata/Anonymouslong/best_practice_pixel/transfer_spatial_fine_tuning_28_29_to_Serbia_R2"
...
print(best_models_directory1)
# checkpoint_path = os.path.join(f'{best_models_directory1}/output_Anonymous_0', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth")
...
def pred_map(ts_stamp, tif_path):
    ...
    output_file = os.path.join(f'./{preprocessing_method}_{model_type}_Direct', f"{ts_stamp}.tif")
...
createFolder(f'./{preprocessing_method}_{model_type}_Direct')
print(f'create Folder {preprocessing_method}_{model_type}_Direct')
...
pred_map('2023-0000002500-0000002000','/mnt/mridata/Anonymouslong/best_practice_pixel/download/Serbia_2023_7day_ln/2023-0000002500-0000002000.tif')

# #%%
# """
# # # Save the lists as a CSV file
# # folder_name = "/data/Anonymous/Anonymousfile/best_practice/data/download/Serbia/Serbian_2023_LC08_7days_ln"
# # name_ls = [f[5:-4] for f in os.listdir(folder_name) if f.endswith('.tif')]
# # tif_ls = [os.path.join(folder_name, f) for f in os.listdir(folder_name) if f.endswith('.tif')]

# # with open(csv_file_path, mode='w', newline='') as file:
# #     writer = csv.writer(file)
# #     writer.writerow(["name", "tif_path"])
# #     writer.writerows(zip(name_ls, tif_ls))
# """
# # Read the CSV file back into two lists
# csv_file_path = "tif_data.csv"
# name_ls = []
# tif_ls = []
...
#         tif_ls.append(row["tif_path"])

# name_ls, tif_ls
# # %%
# for i in range(len(tif_ls)):
#     pred_map(name_ls[i],tif_ls[i])
