...
# Select 400000 samples randomly from the dataset
sample_size = 400000
random_indices = np.random.choice(len(x_data), size=sample_size, replace=False)
sample_size = str(sample_size)+'_'+VI_name
...
for fold_i in range(n_folds): 
    ...
    try:
        ...
        classifier,optimizer = load_checkpoint(os.path.join(f'./output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}.pth"), classifier, optimizer)
        classifier_add,_ = load_checkpoint(os.path.join(f'./output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_add.pth"), classifier_add)
        fc,_ = load_checkpoint(os.path.join(f'./output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_fc.pth"), fc)
