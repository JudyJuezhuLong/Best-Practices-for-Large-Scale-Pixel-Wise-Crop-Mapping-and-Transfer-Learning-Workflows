...
x_add_data = x_add_data.reshape(x_add_data.shape[0],-1)
print(x_data.shape, x_add_data.shape)
x_data = np.concatenate((x_data, x_add_data), axis=1)
print(x_data.shape)
...
# Select 100000 samples randomly from the dataset
sample_size = 100000
...
for fold_i in range(n_folds): 
    ...
    # pickle_file = os.path.join(f'./output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth")
