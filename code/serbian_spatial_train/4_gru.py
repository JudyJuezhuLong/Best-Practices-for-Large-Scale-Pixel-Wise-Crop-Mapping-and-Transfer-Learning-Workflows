...
class GRU(nn.Module):
    ...
    def forward(self, x):
        ...
        fc_flatten = torch.flatten(gru_out, start_dim=1)
...
def main(args: argparse.Namespace):
    ...
    def _eval_perf(net, dataloader, best_loss, device):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            save_checkpoint(net, optimizer, epoch, val_loss, os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth"))
            # save_checkpoint(net, optimizer, epoch, val_loss, os.path.join('./output_Judy_fine_tune_'+scene_id+'site', yy+"_"+preprocessing_method+"_"+model_type+"_"+sample_size+"_best.pth"))
    ...
    for fold_i in range(n_folds): 
        ...
        # save_to_pkl(scaler, os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_scaler.pkl"))
        ...
        checkpoint_path = os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth")
        try:
        ...
        save_to_pth(classifier, os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}.pth"))
        drawAccuracyCurves_(cls_accs_ls, os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_acc.png"))
        drawLossCurves(loss_ls, os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_loss.png"))
        ...
        np.savetxt(os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_confusion_matrix.csv"), confusion_matrix0, delimiter=",")
        ...
        metrics_df.to_csv(os.path.join(f'./output_Judy_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_metrics.csv"), index=False)

#%%
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DANN for Unsupervised Domain Adaptation')
    # dataset parameters
    parser.add_argument('--root', metavar='DIR',default='/data/taoliu/taoliufile/transfer_learning/Transfer-Learning-Library/data/office',
