...
def main(args: argparse.Namespace):
    def build_model(input_feature_size,num_classes,seed=0, hidden_size=256, num_layers=1,bidirectional=False, dropout=0):
    ...
    x_dir_key = 'x_data_dir_'+scene_id1+'_'+preprocessing_method
    y_dir_key = 'y_data_dir_'+scene_id1+'_'+preprocessing_method
    print("X_data1 path:",file_paths[x_dir_key])
    print("label1 path:",file_paths[y_dir_key])
    x_data1 = np.load(file_paths[x_dir_key])
    y_data1 = np.load(file_paths[y_dir_key]).ravel()
    print("X_data1 shape",x_data1.shape)

    x_dir_key = 'x_data_dir_'+scene_id2+'_'+preprocessing_method
    y_dir_key = 'y_data_dir_'+scene_id2+'_'+preprocessing_method
    print("X_data2 path:",file_paths[x_dir_key])
    print("label2 path:",file_paths[y_dir_key])
    x_data2 = np.load(file_paths[x_dir_key])
    y_data2 = np.load(file_paths[y_dir_key]).ravel()
    ...
    for fold_i in range(n_folds): 
        ...
        """
        # get the test data (sample size = 10k)
        x_train_val, x_test, y_train_val, y_test = train_test_split(x_data, y_data, stratify= y_data, test_size=10000, random_state=10)
        # 100
        # sample_size='100'
        # batch_size=32
        # x_train, x_val, y_train, y_val = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=100, random_state=11)
        # 1k
        # sample_size='1k'
        # batch_size=32
        # x_train, x_val, y_train, y_val = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=1000, random_state=12)
        # 10k
        # sample_size='10k'
        # batch_size=int(pow(2, 8))
        # x_train, x_val, y_train, y_val = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=10000, random_state=13)
        # 50k
        # sample_size='50k'
        # batch_size=int(pow(2, 8))
        # x_train, x_val, y_train, y_val = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=50000,  random_state=14)
        # 100k
        # sample_size='100k'
        # batch_size=int(pow(2, 12))
        # x_train, x_val, y_train, y_val = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=100000, random_state=15) 
        
        # 500k
        sample_size='400k'
        batch_size=int(pow(2, 12))
        x_train, x_val, y_train, y_val = train_test_split(x_train_val, y_train_val, stratify= y_train_val, test_size=10000, train_size=400000, random_state=17)
        """
