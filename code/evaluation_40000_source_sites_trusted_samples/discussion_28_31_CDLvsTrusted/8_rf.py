...
for fold_i in range(n_folds): 
    ...
    sc = load_from_pkl(PATH)
    print("Using scaler:", PATH)
    x_test_std = sc.transform(x_test)

    check_point = os.path.join(f'{best_models_directory1}/output_Anonymous_{fold_i}', f"{yy}_{preprocessing_method}_{model_type}_{sample_size}_best.pth")
    rf = load_from_pth_rf(check_point)
    print("Using check point...", check_point)
